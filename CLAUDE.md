...
3. **Run universe generation and simulation synchronously inside a single Vercel serverless function (Route Handler), triggered by the admin — no queue, no separate worker, no polling.** A dedicated queue/worker service (Redis, QStash, a persistent Node process on Railway/Fly) would almost always mean an additional paid service outside the free tier — explicitly rejected under the zero-cost constraint. Instead: the Route Handler does all the work (generate/simulate with the optimized engine) and responds when done, using `export const maxDuration = 60` (the max allowed on Vercel's Hobby plan) for headroom. If any single computation risked approaching the limit, the mitigation is to keep optimizing the engine (more typed arrays, fewer allocations) or split the work into 2-3 sequential calls chained from the admin client itself (e.g. generate Colombia, then Chile, then simulate) — not standing up new infrastructure.
4. **Unified persistence in Postgres**: per-team aggregates (what leaderboards/results actually query) live in normal tables; raw/bulk arrays (full universe, per-policy assignment, 1M-row tariff submissions) live in `bytea` columns of the same Postgres (Neon free tier), not a separate blob service. **Never** store 1M rows as individual relational records (one row per policy) — always as a single binary blob per run/submission.
5. Explicitly rejected: running the unmodified 10M-row generator behind a serverless function as-is (exceeds time/memory); and any job/queue architecture that requires a paid service.
...
/src
  ...
  /app
    ...
    /api/
      ...
      universe/route.ts        # generates Colombia/Chile synchronously, maxDuration=60
      simulation/route.ts       # runs runSimulation synchronously, maxDuration=60
