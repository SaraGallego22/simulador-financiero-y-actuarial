...
| `necesidadesPatrimonioODeuda` | Igual a `capitalComprometido` directamente — una línea de **Pasivo**, no de Activos. Distinta de cero solo una vez que un equipo agotó por completo su portafolio real (Capital Social incluido, ver §5.3) y aun así necesitó más: por construcción, cualquier `capitalComprometido` a esas alturas ya es financiación genuinamente externa (capital nuevo o deuda), nunca un exceso sobre algún remanente de Capital Social — no hay tope que calcular. Cero para la inmensa mayoría de los equipos (ver el detalle completo debajo de esta tabla). |
| `impuestoPorPagar` (fórmula) | Igual al `imp` (Impuesto) del P&G de ese mismo año (§4.1/4.2) — el impuesto de renta ya reconocido contra `patrimonio` (vía `retenido`) pero que todavía no ha salido en efectivo, misma lógica que `rpnd`/`cxp`. El ALM real nunca modela el pago del impuesto como una salida de caja (ver `almSimRealYear()` en `alm.ts`), así que sin esta línea `activos` corría por delante de `Pasivo + Patrimonio` por exactamente el impuesto no pagado — el principal causante del residuo de Año 1/2 antes de esta línea existir. |
...
| `inversiones` | Año 1/2: el `portfolioBookValue` bruto real del ALM de ese año directamente (`AlmRealYearResult.portfolioBookValue`) — ya incluye Capital Social, fondeado en el mismo calendario desde el arranque del Año 1 (§5.3), así que no se suma ningún término aparte encima. Un hecho económico real, nunca el residual que hacía cuadrar el balance en la versión anterior de este motor. Año 3 es la única excepción: como nunca tiene un ALM propio (nunca se simula), no hay ningún hecho independiente que un residual pudiera pisar — todas las demás líneas de su Balance ya son proyecciones mecánicas (`reservasTec` es un runoff proyectado, no reserving genuino; `caja`/`cxc`/`cxp` son las mismas convenciones de % fijo de Año 1/2 sin ALM). Por eso `inversiones` de Año 3 sí se despeja para que `Activos = Pasivo + Patrimonio` cierre exacto — igual que un modelo financiero pro-forma real, que resuelve la línea de caja para cuadrar sus tres estados proyectados. |

**Por qué `necesidadesPatrimonioODeuda` va en Pasivo, no en Activos.** `capitalComprometido` ya se resta una vez de `patrimonio` (equity), y está implícito en un `inversiones` más bajo (activo — el `portfolioBookValue` real del ALM ya refleja lo que tuvo que liquidarse para llegar a necesitar financiación externa). Eso es doble partida contable correcta: la misma plata que salió a cubrir la brecha ya no está invertida ni sigue siendo del dueño. Sumar `necesidadesPatrimonioODeuda` también al lado de **Activos** duplicaría esa misma resta con el signo contrario, no la revertiría; sumarlo al lado de **Pasivo**, en cambio, es coherente con que representa financiación *externa* recién levantada — una obligación nueva, no un activo.

Para Año 1/2, como `caja`/`inversiones` son hechos reales computados de forma independiente (no un residual forzado a cuadrar), `caja + inversiones + cxc` generalmente **no** suma exactamente `Pasivo + Patrimonio`. Antes de que existiera `impuestoPorPagar`, la brecha llegaba a ~20% de `Pasivo + Patrimonio` en algunos equipos de una cohorte real — el impuesto de renta nunca se modelaba como salida de caja real del ALM, así que `activos` quedaba inflado por esa plata que contablemente ya no era de la aseguradora pero que el ALM nunca le entregó a la DIAN. Con `impuestoPorPagar` ya en `pasivo`, lo que queda es mucho más chico (bajo 1-2% de `Pasivo + Patrimonio` en pruebas empíricas contra el motor de reservas real) y viene de que `cxc`/`cxp` asumen un rezago de 30 días/10% en el cobro de prima y el pago de gastos que el ALM real no modela (invierte/paga todo el mismo mes, sin ese rezago). `necesidadesPatrimonioODeuda` **no** existe para absorber esa brecha — solo cubre el caso específico de un portafolio real completamente agotado; el pequeño residual restante de `cxc`/`cxp` es una propiedad conocida de este modelo simplificado para Año 1/2, no un error a corregir línea por línea. Año 3 no tiene ningún residuo: como se explica en la fila de `inversiones` arriba, ahí no hay ningún hecho real de por medio, así que esa línea se despeja para cuadrar exacto.
...
- **El Año 2 real es una continuación genuina del Año 1 real, no una corrida independiente desde cero.** El motor recibe el estado exacto con el que terminó el Año 1 (las mismas posiciones abiertas — que siguen devengando rendimiento y venciendo según su propio plazo — y el mismo capital comprometido acumulado, que nunca se repone solo) y sigue simulando 12 meses más a partir de ahí, con la prima real del Año 2 y el mismo calendario de decisión que el equipo sometió en Día 2 — es el único calendario que existe, no hay una segunda oportunidad de someter uno distinto para el Año 2. El ALM ficticio, en contraste, trata cada año como una hipótesis independiente ("qué habría pasado si este calendario hubiera corrido desde el mes 0").
