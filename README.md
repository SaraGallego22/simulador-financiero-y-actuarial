...
- **Un estado de caja** con seis columnas — **Caja Inicial, Prima Cobrada, Pago Siniestros, Gastos, Vencimientos en caja, Inversión Neta, Caja Final** — contra una **Caja Mínima** obligatoria cada mes (15% de Prima+Siniestros, `FZ.cajaPct`). *Vencimientos en caja* es la única vía por la que el dinero de una inversión regresa a la fila de caja *por su propia decisión* de vencimiento — pero si un mes falta caja, el motor no se queda de brazos cruzados esperando a que algo venza: primero retira de **LIQ** (siempre disponible, sin importar su propio ciclo de vencimiento — es justo para esto), y si LIQ no alcanza, **vende el resto del portafolio antes de tiempo**, empezando por el instrumento menos volátil y dejando ACC como último recurso (`forceLiquidatePortfolio()`). Una `brechaCaja` genuina (caja mínima incumplida) ya solo ocurre si ni LIQ ni el resto del portafolio combinado alcanzan — no basta con que la caja de mano se agote.
- **Una evolución del valor del portafolio** — Saldo Inicial, Rendimiento devengado, Saldo Final — separada del estado de caja anterior, porque responde una pregunta distinta: no "¿hay caja suficiente?" sino "¿cuánto vale lo que llevamos invertido?". `Saldo Final = Saldo Inicial + Rendimiento − Vencimientos en caja − Inversión Neta` (un mes con superávit invertido tiene Inversión Neta negativa, así que ese término *suma* al saldo; un mes con retiro de LIQ o venta forzada para cubrir una brecha la *resta*) — es una identidad exacta, verificada en `alm.test.ts`.

De esa simulación salen cuatro notas (`scoreFinanciero()`):

- **Cumplimiento de Caja Mínima (35%)** — ya no mide "¿se agotó la caja de mano?" sino insolvencia genuina: se penaliza tanto la **brecha máxima** (el peor mes) como la **brecha promedio acumulada** en los 60 meses, pero ambas solo cuentan lo que sobrevivió después de agotar LIQ *y* vender el resto del portafolio — ver la nota de venta forzada abajo para el costo de llegar a ese punto.
...
- **Venta forzada de portafolio (20%)** — el castigo por verse obligado a vender antes de tiempo, y no es un castigo plano: pesa el monto vendido por la volatilidad *del instrumento vendido* (`ventaForzadaVolWeighted`, normalizado contra el peor caso posible — vender toda la Caja Mínima acumulada en ACC — para dar un score 0-100). Vender ACC bajo presión sale mucho más caro en la nota que vender CDT90 o TES por el mismo monto; vender LIQ no cuenta en absoluto, porque ese es exactamente su trabajo. Un equipo que nunca necesitó vender nada antes de tiempo obtiene 100 aquí sin importar qué tan volátil sea el resto de su estrategia — esta nota mide *disciplina de liquidez*, no el riesgo del portafolio en sí (eso ya lo mide el Rendimiento ajustado por riesgo de arriba).
