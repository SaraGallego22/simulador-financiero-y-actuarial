...
- **Severidad media** — `calcMediaSev()` — proporcional al valor asegurado del vehículo, con factores por tipo de vehículo, zona y antigüedad. El siniestro individual se muestrea de una **Gamma** con esa media (forma fija), lo que da una cola derecha realista (muchos siniestros pequeños, pocos grandes). Los siniestros del **Año 2** (`generateYear2Claims()`) usan el mismo modelo de severidad pero con un año adicional de inflación de costo de siniestros (`CLAIMS_INFLATION_ANNUAL = 9%` en `src/domain/generation/constants.ts`) aplicado como un multiplicador plano sobre la media, antes del sorteo Gamma — la frecuencia (λ) no cambia por esto, solo el costo de un siniestro que sí ocurre. No se le comunica esta tasa a los equipos en ningún lado del producto (ver §1.1 para el reto de valor presente relacionado, sobre el dataset Chile).
...
- **Tendencia real dentro de Chile mismo (para la proyección del Año 3, ver §4.2)** — a diferencia de la severidad estática que usaban las tres cotas de 2021-2023 antes, `generateChile()` ahora crece la severidad `CHILE_REAL_SEVERITY_GROWTH_ANNUAL = 3%` cada año (aplicado a `baseUf` antes del sorteo Gamma, igual que `CLAIMS_INFLATION_ANNUAL` en Colombia). Como está en UF — una unidad ya indexada a la inflación chilena —, esa tendencia es un crecimiento **real** de costo (repuestos/mano de obra), no inflación disfrazada. No se publica: la pista para un equipo que la mida por su cuenta (comparando severidad promedio 2021 vs. 2022 vs. 2023 en el CSV de Chile) es que, combinada con su propia estimación de la inflación general de Colombia, debería acercarlos a `CLAIMS_INFLATION_ANNUAL = 9%` — el mismo número, oculto, que el motor ya usa internamente para inflar los siniestros de Colombia de Año1→Año2 (y, ahora, de Año2→Año3). Esa combinación es multiplicativa, no una suma: `(1+CLAIMS_INFLATION_ANNUAL) = (1+inflación general)×(1+CHILE_REAL_SEVERITY_GROWTH_ANNUAL)` — el motor mismo nombra el despeje de esa ecuación como `GENERAL_INFLATION_ANNUAL ≈ 5.83%` (`src/domain/generation/constants.ts`), que además ya no es solo una pista reconstruible: alimenta directamente cómo se le muestra a un equipo el rendimiento de TES UVR (ver §5, la tabla de instrumentos).
...
Al cierre del Año 2, `computeDevelopment()` compara lo **realmente emergido** (siniestros del Año 1 avisados tarde, ya en el Año 2) contra lo que el IBNR había estimado — esa diferencia (`development.development`) es una ganancia/pérdida de desarrollo de reservas, su propia línea en el P&G del Año 2 (ver §4), nunca mezclada con el costo de siniestros del Año 2 mismo. Esto es deliberado: un equipo puede tarifar bien pero reservar mal (o viceversa), y ambas cosas se califican por separado.
...
Construido por `pyg(primaEmitida, rpndLiberada, costo, desarrollo, rinv, reservas)`, con estos insumos. Año 1 no tiene año anterior del que liberar RPND, ni nada que desarrollar todavía, así que ninguna de esas dos líneas aparece aquí (sí en Año 2, ver §4.2):
...
| `rt` (Resultado Técnico, fórmula) | `primaDevengada − costo − gadq − gcom` — deliberadamente sin el gasto administrativo, que ahora tiene su propia línea (ver `ri`). |
...
| `uai` (fórmula) | `ri + rinv` (ya no `rt + rinv`). |
...
| `costo` | `development.ultY2` — el costo de los siniestros **propios del Año 2** únicamente, en base fecha de accidente, igual que Año 1. Ya **no** incluye el desarrollo de Año 1 (antes `costoCal = ultY2 + development.development`, mezclando dos años de accidente distintos en una sola línea) — ese ajuste ahora es su propia línea (`desarrollo`, abajo). Los siniestros del Año 1 avisados tarde ya estaban en el `costo` del Año 1 desde el principio (`year1.claimsAmount` ya era el último verdadero, avisado + IBNR — ver §3): sumarlos otra vez aquí los habría contado dos veces. |
| `desarrollo` (Desarrollo de reservas A1) | `development.development` — cuánto más (o menos) de lo esperado como IBNR del Año 1 terminó emergiendo realmente; puede ser negativo. Es una ganancia/pérdida de **reservas**, no de siniestros del Año 2 — existe porque la reserva que se booked al cierre del Año 1 (`bookedReserveEndY1`) fue una *estimación* (el factor de desarrollo de mercado, §3), y esta línea es el ajuste una vez la realidad la desmiente. No es una fórmula de otras líneas — depende de la propia estimación de reservas que el equipo reportó en el Balance (§4.3) y de lo que realmente emergió, así que se califica como un hecho/estimación primario, contra el valor verdadero del motor. |
...
| `costo` | Únicamente el siniestro **propio de Año 3, proyectado**: frecuencia igual a la observada en Año 2 (`development.claimCountY2 / year2.insuredCount`) por severidad de Año 2 inflacionada un año más por `CLAIMS_INFLATION_ANNUAL` (9%, el mismo número — no uno nuevo — que ya infla los siniestros de Colombia de Año1→Año2; ver §1.1 para por qué esto no es una segunda tasa distinta). **Ya no** se le suman `devTailY1InY3`/`devTailY2InY3` (las colas reales de pago de Año 1/Año 2 que caen en el calendario de Año 3): ese dinero ya se reconoció como costo incurrido en el P&G de su propio año de accidente — sumarlo aquí también sería contarlo dos veces. Esas colas siguen existiendo, solo que ahora son puramente un movimiento de reserva (ver `reservas` abajo), sin ningún efecto en el P&G. |
| `gadq` / `gcom` / `rt` / `gadm` / `ri` (fórmula) | Mismas fórmulas que Año 1/Año 2, sobre la `primaDevengada`/`costo` de arriba — **sin** línea de desarrollo: la reserva de Año 2 nunca fue una estimación tipo-mercado como la de Año 1 (§3 no le calcula un `expectedIBNR` propio), así que no hay nada que "desmentir" un año después. |
...
| `pasivo` (fórmula) | `reservasTec + rpnd + cxp` — antes solo `reservasTec + cxp`; ahora la RPND es un pasivo aparte, junto a las reservas técnicas y las cuentas por pagar. |
...
| `inversiones` (fórmula) | `reservasTec + rpnd + cxp + patrimonio − caja − cxc` — el residual que hace cuadrar el balance (activos = pasivos + patrimonio), no un valor de mercado del portafolio. Antes `reservasTec + cxp + patrimonio − caja − cxc`; ahora la RPND también entra al residual. |
...
**El desfase de un mes que se corrigió en `stepMonth()`.** Cada mes, el motor acumulaba rendimiento sobre las posiciones abiertas *antes* de procesar los vencimientos y la inversión del excedente de ese mismo mes — así que una posición recién creada (por reinversión de un vencimiento o por invertir plata nueva) se perdía su propio primer mes de rendimiento, cada vez que se creaba. Para un tramo de 12 meses eso es solo ~8% de rendimiento perdido, pero para uno de 3 meses (CDT90, que no tiene otra opción — su plazo es fijo) es ~33%, y para un vencimiento personalizado de 1 mes es el 100%: rendimiento realizado cero, sin importar el instrumento. Se verificó de forma aislada: el mismo instrumento, mismo rendimiento nominal, pasaba de 0% realizado (`durationM=1`) a menos de medio punto del nominal (`durationM=60`, nunca vence dentro del horizonte) — un artefacto puramente mecánico del orden de las operaciones, sin relación con el riesgo real del instrumento. Ahora la acumulación corre *después* de procesar vencimientos e inversión nueva, así que cada posición devenga exactamente los meses que estuvo abierta — ni uno menos.
...
- **Objetivo por día** — mezcla actuarial/financiero (peso configurable): el actuarial incluye la calidad de la tarifa más cualquier entregable numérico de ese perfil; el financiero, los entregables financieros más la nota ALM/analítica cuando aplica. La calidad de la tarifa se mide sobre el mismo RT que usa `finBench()` (`prima − siniestros − gastos de adquisición/comisión`, `RT_EXPENSE_PCT` en `src/domain/finance/constants.ts` — deliberadamente sin gasto administrativo, que ahora es su propia línea, Resultado Industrial, ver §4.1) para que "RT" signifique lo mismo en todo el modelo. A propósito, sobre prima **emitida** cruda, no devengada — esta nota es sobre calidad de tarificación/suscripción, no sobre la contabilidad de reconocimiento de ingreso que sí importa en el P&G completo (§4.1):
