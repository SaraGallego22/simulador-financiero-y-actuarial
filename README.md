...
En los dos años, `capacityByTeamId`/`cuotaPct` son un techo duro en la fase de redistribución: ningún equipo es forzado a recibir más pólizas que su límite por solvencia, ni siquiera en el caso extremo en que la capacidad sumada de todos los equipos no alcance a cubrir todo el universo — esas exposiciones sobrantes quedan sin asegurar (`assignment = -1`) en vez de forzarse sobre algún equipo. La única excepción deliberada es el piso de `MIN_POLICIES_PER_TEAM` (que evita que un equipo quede en cero por precio poco competitivo): ese piso es incondicional y puede llevar a un equipo por encima de su propio límite de capital — ahí se prioriza que el equipo siga en el juego sobre el techo de solvencia. Para completarlo, primero se usan las exposiciones que quedaron sin asegurar (no le quitan nada a otro equipo) y solo si no alcanzan se le quitan pólizas (las más baratas) a los equipos con excedente.
...
| `cxp` (Cuentas por pagar, fórmula) | `(30 × gastos) / 365` (`FZ.diasRotacionCxp = 30`), con `gastos = GASTOS_TOTAL_PCT × primaEmitida` (comisión + adquisición + administración) de ese año — la misma fórmula de días de cartera que `cxc`, pero sobre gastos en vez de prima: 30 son los días que la aseguradora tarda, en promedio, en pagarle a sus proveedores (comisiones e intermediación) lo que les debe. Reemplaza el antiguo 10% plano de `primaEmitida` (`FZ.cxpPct`). Para Año 1, `primaEmitida` es la que se reportó un día antes, en Día 2 (§4.1) — la única otra fórmula (junto con `rpndLiberada` de Año 2) que cruza de un día a otro. |
...
**La nota que se califica se normaliza contra lo que la simulación realmente logra, no contra la fórmula nominal de arriba.** `RISK_ADJUSTED_YIELD_MIN`/`MAX` (`src/domain/finance/alm.ts`) son el Sharpe ratio (menos el descuento de concentración de §5.2) **realizado** — no el de esta tabla — de dos portafolios de referencia corridos completos a través de `almSim()`/`scoreFinanciero()`. Recalibrados cuando `HORIZON` pasó de 48 a 96 y el fondeo nocional se subió por `GASTOS_TOTAL_PCT` (§5.1): ambos cambios mueven `avgPV`/`effYield`/`avgPortfolioVol` de cualquier portafolio de referencia, así que el piso/techo viejo (−0.07/1.405) ya no describía el piso/techo real de este motor.
- **Piso ≈ −0.058: 100% ACC**, `durationM=ACC_ROLL_M`, "repeat" indefinido — misma referencia de siempre, misma razón: LIQ **es** la tasa libre de riesgo, así que su propio Sharpe es ≈0 por construcción y está exenta del descuento de concentración, mientras que ACC 100% paga el descuento completo (μ=0.5) sobre un Sharpe crudo que no alcanza a absorberlo. Re-verificado contra cada instrumento individual del menú bajo el motor nuevo (CDT90/TES1/TES3/TESUVR8 100% puros siguen dando positivo).

  Existe un número todavía peor, y se descarta a propósito: una astilla de ACC dentro de casi puro LIQ (1% ACC / 99% LIQ) da −0.12, y sigue empeorando — pasa de −0.5 — mientras la astilla de ACC se hace más chica. Es un artefacto del Sharpe ratio (el denominador, `avgPortfolioVol`, colapsa hacia la volatilidad casi nula de LIQ más rápido que el numerador), no una decisión genuinamente peor: en esa esquina, *más* ACC da mejor nota, no peor, justo lo contrario de lo que "peor decisión" debería significar. Calibrar el piso ahí aplastaría cualquier decisión mala real (ACC concentrado, capital muerto, apuestas propensas a venta forzada) contra una banda angosta cerca de 100.
- **Techo ≈ 1.391: {LIQ 0%, CDT90 41.5%, TES1 43%, TES3 4.5%, TESUVR8 11%}**, "repeat" indefinido — hallada por búsqueda en grilla sobre el simplex de 5 instrumentos (ACC excluido; una verificación puntual confirma que cualquier porción de ACC solo empeora esta referencia) y confirmada contra una segunda búsqueda independiente para descartar que fuera un artefacto de la grilla. CDT90/TES1 siguen dominando la mezcla — tienen el mejor Sharpe individual del menú (ver la tabla de arriba) — diluidos lo justo con TESUVR8/TES3 para que su correlación imperfecta baje un poco más la volatilidad de la combinación.

  Lo que sí cambió frente a la calibración anterior: LIQ pasó de un peso real (~20%) a 0%. Antes, apoyarse en LIQ para diluir `portfolioConcentrationRatio()` (queda fuera de la base del índice, así que baja la concentración gratis) compensaba su rendimiento casi nulo; con el horizonte más ancho y el fondeo grosseado — más meses para que CDT90/TES1 compongan, un pool nocional más grande — esa compensación deja de convenir.
...
`almSim()` simula mes a mes (108 meses: 12 de fondeo + 96 de corrida, para cubrir toda la cola de `lib.L[]`) dos vistas separadas del mismo portafolio:
...
Todo lo anterior corre sobre una **hipótesis deliberadamente irreal**: que la Prima Cobrada de cada mes es 1/12 de `(reserva + pagos del Año 1) ÷ (1 − GASTOS_TOTAL_PCT)` — la misma forma en que se calcula una prima comercial real (Prima Comercial = Prima Pura ÷ (1 − %Gastos − %Utilidad), sin margen aquí), para que lo que quede después de pagar el 25% de gastos alcance exactamente para fondear la reserva, ni más ni menos. Sin ese ajuste, el ALM ficticio le faltaría caja por construcción — no por ninguna decisión del equipo, sino porque estaría gastando el 25% de un ingreso ya calculado justo al límite de lo que la reserva necesita. En la realidad, la prima de un equipo es la que **el mercado le pagó** por su tarifa (Día 1/§2), y casi nunca coincide con su reserva. Este ALM "ficticio" no es un error del modelo — es **a propósito**, y sigue siendo el único que se califica (§5.2) y el único que ve el equipo.
...
- **El ALM real solo corre 12 meses por año, nunca los 108 del ficticio.** Su único propósito es alimentar el P&G/Balance real de *ese* año — no tiene sentido simular 96 meses de más cuando nada los va a usar. El ALM ficticio, en cambio, corre 108 meses completos por año (12 de fondeo + 96 de corrida) porque eso es lo que su propia nota (§5.2) necesita evaluar — 96, no 48, para que le alcance para pagar toda la cola de `lib.L[]` (§3), que ahora se extiende igual de lejos por el mismo rezago de aviso largo.
...
- **El siniestro que financia cada año real es distinto al del ficticio.** El Año 1 real se financia contra los siniestros propios del Año 1 (`liabilityYear1.payY1`, los mismos 12 meses que usa el ficticio en su propia fase de fondeo). El Año 2 real se financia contra la **suma de dos cosas**: el desarrollo del Año 1 que emerge en el Año 2 (los primeros 12 meses de `liabilityYear1.L[]` — la misma reserva que el ficticio arrastra indefinidamente, aquí usada solo por 12 meses) *más* los siniestros propios del Año 2 en su propio primer año (una `LiabilitySchedule` nueva, calculada igual que la del Año 1 pero sobre los siniestros de `generateYear2Claims()`). El ALM ficticio, en cambio, usa siempre la reserva del Año 1 para todo su horizonte de 96 meses.
...
**Importante para no confundir qué ALM alimenta qué**: la nota de ALM del Día 2 (§5.2, lo que ve el equipo) se califica con el ALM **ficticio** (`almSim()`/`scoreFinanciero()`, 108 meses, independiente por año) contra el calendario de Día 2 exclusivamente — el "Portafolio 2028" opcional de Día 3 (§5.0bis) nunca la afecta. Pero `finBenchHelper.ts` — la plomería que alimenta a `finBench()` (§4) — corre el ALM **real** (`almSimRealYear()`, 12 meses, Año 2 continuando el Año 1) específicamente para eso: benchmarquear un entregable real (Resultado de Inversiones, Balance, Solvencia) contra el ALM ficticio sería comparar contra un escenario hipotético en el que el equipo nunca estuvo. Es también el ALM real, y no el ficticio, el que usa el "Portafolio 2028" cuando existe (§5.0bis). Son dos motores distintos, para dos propósitos distintos — ninguno alimenta al otro.
