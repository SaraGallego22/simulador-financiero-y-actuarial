...
| `inversiones` | Año 1/2: el `portfolioBookValue` bruto real del ALM de ese año directamente (`AlmRealYearResult.portfolioBookValue`) — ya incluye Capital Social, fondeado en el mismo calendario desde el arranque del Año 1 (§5.3), así que no se suma ningún término aparte encima. Año 3 (sin ALM propio) carga hacia adelante el `inversiones` real de Año 2, sumándole exactamente el mismo crecimiento de `patrimonio` de Año 2 a Año 3 (`bal2.inversiones + (patrimonioY3 − bal2.patrimonio)`) — no la tendencia bruta Año1→Año2 del portafolio mismo, que está dominada por un año completo de flujo de caja de prima (el ALM real reinvierte cada mes), uno o dos órdenes de magnitud más grande que la utilidad neta retenida de un solo año; extrapolar ese delta bruto sobrestima el crecimiento de Activos muy por encima del crecimiento real de Patrimonio. Cae a `CAPITAL_SOCIAL` plano solo cuando no hay ningún ALM real del que partir (ni siquiera Año 2). Un hecho económico real, nunca el residual que hacía cuadrar el balance en la versión anterior de este motor. |
