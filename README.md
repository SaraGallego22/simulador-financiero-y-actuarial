...
| `p2_ajusteSiniestralidad` (Ajuste de siniestralidad A1, fórmula — con un término `useTrueValue`) | No es una línea del `PnL` que calcula `finBench()` — vive solo como concepto calificable en `concepts.ts`, evaluado contra `year1.claimsAmount` (el verdadero Costo de Siniestros A1) menos `p1_costo` (la línea que el equipo mismo reportó para eso en Día 2). Corrige la utilidad del Año 2 por un error en la propia estimación de siniestralidad del Año 1: positivo si el equipo la subestimó, negativo si la sobreestimó. Es la única línea que mezcla un valor verdadero del motor con una línea que el equipo reportó un día antes (`FormulaTerm.useTrueValue`, `src/domain/grading/concepts.ts`) — el `rt` de abajo sigue siendo una fórmula ordinaria de otras líneas, ya reportadas ese mismo día. |
| `gadq` (fórmula) | `4% × primaEmitida` del Año 2. |
| `gcom` (fórmula) | `15% × primaEmitida` del Año 2. |
| `rt` (fórmula) | `primaDevengada − costo − p2_ajusteSiniestralidad − gadq − gcom` — el ajuste de siniestralidad resta aquí, junto al costo, antes de llegar al resultado técnico. |
