...
`necesidadesPatrimonioODeuda` **no** existe para absorber esta brecha — solo cubre el caso específico de un portafolio real completamente agotado. Año 3 tampoco tiene residuo: como se explica en la fila de `inversiones` arriba, ahí no hay ningún hecho real de por medio, así que esa línea se despeja para cuadrar exacto.
