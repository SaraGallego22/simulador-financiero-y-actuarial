...
El **Año 3 no se simula** — no hay un tercer mercado ni un tercer ALM. Cuando `development` trae los campos de cola de Año 3 (ver §3) y además hay `insuredCount` de Año 1/Año 2 y la retención real de Año 2 (`year2Retention`, de `TeamSimResult.extra`), `p3` se construye igual que `p2`: mezclando dato real donde existe con una única pieza proyectada donde no puede existir todavía.

| Línea | De dónde sale |
|---|---|
| `prima` | Pólizas retenidas + nuevas, no una tasa de crecimiento plana: `retentionRate = year2Retention.retainedCount / year1.insuredCount` (la retención real Año1→Año2) aplicada hacia adelante sobre el libro de Año 2 (`retainedPolicies3 = retentionRate × year2.insuredCount`), más las mismas pólizas nuevas que entraron en Año 2 (`newPolicies3 = year2Retention.newCount`) — `prima3 = (retainedPolicies3 + newPolicies3) × (prima2 / insuredCount2)`. |
| `costo` | Suma de tres piezas: el **desarrollo real y exacto** de Año 1 (`development.devTailY1InY3`, su 3ª y última cola) y de Año 2 (`development.devTailY2InY3`, su 2ª cola) — ambos calculados con el mismo kernel de pagos de §3, un paso más adelante, sobre los mismos reclamos reales ya conocidos — más un siniestro **propio de Año 3, proyectado**: frecuencia igual a la observada en Año 2 (`development.claimCountY2 / year2.insuredCount`) por severidad de Año 2 inflacionada un año más por `CLAIMS_INFLATION_ANNUAL` (9%, el mismo número — no uno nuevo — que ya infla los siniestros de Colombia de Año1→Año2; ver §1.1 para por qué esto no es una segunda tasa distinta). |
| `reservas` | `development.osY1endY3 + development.osY2endY3` (las mismas colas reales de arriba, la parte que sigue sin pagarse) más lo que queda abierto del siniestro propio de Año 3 proyectado en su propio primer año de desarrollo (45% de `siniestrosNuevosAño3`, el mismo 1−`DEV_FRAC[0]` de §3). |
...
| `gadq` / `gcom` / `gadm` / `rt` / `uai` / `imp` / `uneta` | Mismas fórmulas que Año 1/Año 2, sobre la `prima`/`costo` de arriba. |

Balance/capital comprometido de Año 3 no cambian: siguen las mismas fórmulas de caja/cxc/cxp (§4.3) y el mismo capital comprometido acumulado de Año 2 hacia adelante, sin asumir nueva erosión — no era ahí donde el modelo era superficial.

**Fallback** (cuando falta cualquiera de los insumos de arriba — `development` sin los campos de Año 3, o sin `insuredCount`/`year2Retention`): `p3` cae de vuelta a la proyección plana original, sin cambios — `prima3 = prima2 × 1.06`, `costo3 = costo2 × 1.06`, `reservas3 = reservas2 × 1.06`, `rinv3 = reservas3 × portYield` (`FZ.growth3 = 6%`).
...
**Lo que un equipo ve para TES UVR no es el 12.0% nominal de esta tabla.** `displayYield()` (`src/domain/finance/instruments.ts`) es una capa puramente de presentación — nunca tocada por el motor de simulación ni por ningún cálculo (ALM, Markowitz, `scoreFinanciero()`, `finBench()` siguen usando `ins.yield = 12.0%` sin cambios) — que muestra el rendimiento de TES UVR **neto de inflación general** en vez de nominal: `(1+12.0%)/(1+GENERAL_INFLATION_ANNUAL) − 1 ≈ 5.83%` (`src/domain/generation/constants.ts` — descomposición multiplicativa tipo Fisher a partir de `CLAIMS_INFLATION_ANNUAL` y `CHILE_REAL_SEVERITY_GROWTH_ANNUAL`, no una resta). Es una trampa deliberada, del mismo estilo que las de §1.1: TES UVR está indexado a inflación, así que un retorno real por encima de inflación es su verdadera ventaja — mostrarlo junto a cinco instrumentos cotizados en términos nominales sin más contexto invita a compararlo en la misma base sin darse cuenta de que no lo es. Un equipo que no repare en esto ve a TES UVR (5.83% mostrado) rindiendo *menos* que CDT90 (9.5%) o cualquier TES nominal, y puede subponderarlo sin razón — la cifra correcta para compararlo de igual a igual contra el resto del menú (todos nominales) requiere sumarle de vuelta la inflación general, exactamente el mismo tipo de razonamiento que exige la brecha temporal de Chile. No se revela como número en ninguna vista de equipo (guías, formularios, CSV descargable) — todas usan `displayYield()` de forma consistente, así que el número "equivocado" es el mismo en todas partes, no una inconsistencia entre pantallas.
