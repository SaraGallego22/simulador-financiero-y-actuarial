...
#### 5.3 · El ALM es ficticio — cómo deducir el ALM real

Todo lo anterior corre sobre una **hipótesis deliberadamente irreal**: que la Prima Cobrada de cada mes es exactamente 1/12 de `reserva + pagos del Año 1` — es decir, que la prima cobrada siempre alcanza exactamente para fondear la reserva, ni más ni menos (ver la nota histórica en `almSim()`'s docstring). En la realidad, la prima de un equipo es la que **el mercado le pagó** por su tarifa (Día 1/§2), y casi nunca coincide con su reserva. Esta ALM "ficticio" no es un error del modelo — es **a propósito**: el ejercicio pedagógico es que cada equipo, al terminar el Día 1 (y de nuevo al terminar el Día 2, para el Año 2), mire los resultados de este ALM ficticio y **deduzca a mano cómo se vería su ALM real** con su prima real, para completar dos entregables reales:

1. **El resultado de inversiones del P&G real** (que cada equipo sube como entregable del día siguiente): si la prima real de un equipo es mayor o menor que la reserva, el monto disponible para invertir cada mes cambia proporcionalmente — el rendimiento porcentual (`portYield`/`effYield`) de su árbol de decisiones sigue siendo válido, pero el monto en pesos que ese rendimiento genera hay que reescalarlo a la prima real, no a la prima ficticia del ALM.
2. **Los activos, pasivos y patrimonio del Balance real** (Día 2/3): el mismo reescalamiento aplica a cuánto queda efectivamente invertido, y si el equipo tuvo que comprometer Capital Social en el ALM ficticio, esa señal (¿fue un mes puntual y pequeño, o una erosión crónica?) es la pista de cuánto colchón de capital le haría falta bajo su propia estructura real de primas.

`finBench()` (§4) es el motor de referencia que hace exactamente este reescalamiento del lado del evaluador — usa la prima real (`year1.totalPremium`) para el P&G, pero el `portYield`/`invInc` y el capital comprometido del ALM ficticio para el resultado de inversiones y la erosión de patrimonio — así que el número que un equipo reporte a mano debería aproximarse a lo que `finBench()` calcula, no copiarlo directamente del ALM ficticio sin ajustar.
...
- **"Ignorar que el ALM es ficticio y copiar sus números directo al Balance/P&G real"** — ver §5.3: el resultado de inversiones y el patrimonio que hay que reportar en los entregables reales deben reescalarse a la prima real del equipo, no tomarse literalmente del ALM ficticio (que asume prima = reserva, una hipótesis que casi nunca se cumple).
