...
Para Año 1/2, como `caja`/`inversiones` son hechos reales computados de forma independiente (no un residual forzado a cuadrar), `caja + inversiones + cxc` generalmente **no** suma exactamente `Pasivo + Patrimonio` — para un equipo con un ALM real sano, la brecha es pequeña (bajo 1-2% de `Pasivo + Patrimonio` en pruebas empíricas contra el motor de reservas real), y viene de que `caja`/`cxp`/`rpnd` son convenciones simplificadas de porcentaje fijo mientras `caja`/`inversiones` ya reflejan la simulación mensual real del ALM. `necesidadesPatrimonioODeuda` **no** existe para absorber esa brecha — solo cubre el caso específico de un portafolio real completamente agotado; el pequeño residual restante es una propiedad conocida de este modelo simplificado para Año 1/2, no un error a corregir línea por línea. Año 3 no tiene este residual: como se explica en la fila de `inversiones` arriba, ahí no hay ningún hecho real de por medio, así que esa línea se despeja para cuadrar exacto en vez de dejar una brecha.
