...
| Línea | De dónde sale |
|---|---|
| `caja` | Año 1/2: el `cajaFinal` real de diciembre del ALM real de ese año (`AlmRealYearResult.cajaFinalAnio`, ver §5.3) — la Caja Mínima que ese ALM realmente sostuvo al cierre, no un porcentaje de la prima. Año 3 (sin ALM propio, nunca simulado) cae de vuelta a `15% × primaEmitida` (`FZ.cajaPct`), igual que el resto del P&G proyectado. Un hecho/estimación primario, no una fórmula de otras líneas del Balance — el equipo nunca ve su ALM real (solo el ficticio, ver §5.0), así que lo estima con su propio criterio, igual que `reservasTec`. |
...
| `necesidadesPatrimonioODeuda` | `max(0, capitalComprometido − CAPITAL_SOCIAL)` — una línea de **Pasivo**, no de Activos. Distinta de cero solo una vez que un equipo comprometió más Capital Social del que tenía disponible al arrancar el año: ese exceso tuvo que salir de financiación fresca (capital nuevo o deuda) ajena al Capital Social original, así que se reconoce como una obligación aparte. Cero para cualquier equipo que nunca agotó su Capital Social — el caso común (ver el detalle completo debajo de esta tabla). |
| `pasivo` (fórmula) | `reservasTec + rpnd + cxp + necesidadesPatrimonioODeuda` — la RPND y `necesidadesPatrimonioODeuda` son pasivos aparte, junto a las reservas técnicas y las cuentas por pagar. |
...
| `inversiones` | Año 1/2: el `portfolioBookValue` bruto real del ALM de ese año (`AlmRealYearResult.portfolioBookValue`, la reinversión del flujo de caja de la prima — nunca el Capital Social, ver §5.3) más el Capital Social que ese equipo **no** ha tenido que comprometer (`max(0, CAPITAL_SOCIAL − capitalComprometido)` de ese año, nunca negativo — ver la nota debajo). Año 3 (sin ALM propio) cae solo al segundo término, sin sumar ningún valor de portafolio proyectado. Un hecho económico real, nunca el residual que hacía cuadrar el balance en la versión anterior de este motor. |

**Por qué `necesidadesPatrimonioODeuda` va en Pasivo, no en Activos.** `capitalComprometido` ya se resta una vez de `patrimonio` (equity) y una vez del término de Capital Social de `inversiones` (activo, acotado en 0 — Capital Social no puede fondear un faltante que no tiene). Eso es doble partida contable correcta: la misma plata que salió a cubrir la brecha ya no está invertida ni sigue siendo del dueño. Una vez `capitalComprometido` supera `CAPITAL_SOCIAL`, el término de `inversiones` se queda acotado en 0 — no puede compensar el exceso volviéndose negativo — así que ese exceso necesita su propia línea. Verificado empíricamente (no solo por construcción): sumar `necesidadesPatrimonioODeuda` al lado de **Activos** duplicaría el efecto de `capitalComprometido` justo cuando supera `CAPITAL_SOCIAL` (el signo se invierte al pasar por el tope de `inversiones`); sumarlo al lado de **Pasivo**, en cambio, mantiene el desfase Activos-vs-Pasivo+Patrimonio idéntico exista o no exceso — la línea nunca interactúa con (ni empeora) ningún otro cálculo del Balance, solo reporta el hecho de que el equipo gastó más Capital Social del que tenía.

Como `caja`/`inversiones` ahora son hechos reales computados de forma independiente (no un residual forzado a cuadrar), `caja + inversiones + cxc` generalmente **no** suma exactamente `Pasivo + Patrimonio` — para un equipo con un ALM real sano, la brecha es pequeña (bajo 1-2% de `Pasivo + Patrimonio` en pruebas empíricas contra el motor de reservas real), y viene de que `caja`/`cxp`/`rpnd` son convenciones simplificadas de porcentaje fijo mientras `caja`/`inversiones` ya reflejan la simulación mensual real del ALM. `necesidadesPatrimonioODeuda` **no** existe para absorber esa brecha — solo cubre el caso específico de Capital Social agotado; el pequeño residual restante es una propiedad conocida de este modelo simplificado, no un error a corregir línea por línea.
...
Por separado, `almNAV()` valora el portafolio y la reserva a valor de mercado bajo escenarios de tasa (base/alza/baja) — un diagnóstico de sensibilidad a tasa, informativo (no alimenta la solvencia, que usa la volatilidad realizada y el capital comprometido en su lugar, ver §4). Usa la asignación inicial como foto del balance en la fecha de valoración, no el árbol completo de reinversión. Es el precedente directo del riesgo de tasa/inflación calificado del Día 4 (§5.7), que sí valora el árbol real completo, en el cierre real de Año 2.
...
**El mismo ALM real también alimenta `caja`/`inversiones` del Balance (§4.3)**, vía dos campos más de `AlmRealYearResult`: `cajaFinalAnio` (el `cajaFinal` de la última fila de diciembre — un saldo puntual, no un flujo anual) y `portfolioBookValue` (el valor bruto del portafolio a fin de año, `realBookSum` sin descontar `capitalComprometidoAcumulado` — deliberadamente bruto, porque el capital comprometido nunca fue parte de este portafolio para empezar, solo salió de Capital Social directamente). Ninguno de los dos incluye Capital Social — el ALM real, igual que el ficticio, solo reinvierte flujo de caja de prima; el Capital Social no comprometido que sigue invertido se suma aparte en `balance()` (§4.3), nunca dentro de esta corrida.
...
- **Curva real** (`realCurveRate()`): el menú solo tiene un instrumento genuinamente indexado a inflación (TESUVR8), así que no puede definir una *forma* de curva real por sí solo — solo un punto de anclaje, a su propio plazo de 96 meses (`displayYield(TESUVR8)`, el mismo "Inflación + X%" que el equipo ya ve en el menú). La curva real reutiliza la forma de la curva nominal (mismo movimiento relativo entre plazos — la misma simplificación de inflación constante que ya usa el resto del motor en cualquier otro lugar donde necesita un solo número de inflación) desplazada por el único spread que la hace pasar exactamente por el yield real de TESUVR8 en el mes 96: `REAL_CURVE_SPREAD = nominalCurveRate(96) − displayYield(TESUVR8)`.
- **Curva de inflación implícita** (`inflacionCurveRate()`): Fisher aplicado punto a punto entre las dos curvas anteriores, `(1+nominal(t))/(1+real(t)) - 1` — varía levemente por plazo aunque el spread real-nominal sea constante, porque Fisher es multiplicativo, no aditivo. Nunca se entrega directamente: el equipo la despeja de las otras dos.

Los dos choques se aplican a la curva **entera**, no a un solo punto:

- **Riesgo de tasa** choca la curva **real** en todos sus plazos (±20%/-15%, misma convención relativa que `ALM_TASA_ALZA/BAJA` de §5.3) manteniendo la inflación implícita de cada plazo fija — la curva nominal se mueve como consecuencia mecánica de Fisher en cada plazo, así que este choque afecta tanto a TESUVR8 (vía la curva real directamente) como a todo lo demás (vía la curva nominal re-derivada).
- **Riesgo de inflación** choca la curva de inflación implícita en todos sus plazos (misma convención ±20%/-15%) manteniendo la curva real fija — solo la curva nominal re-derivada se mueve, así que TESUVR8 queda intacto mientras todo lo nominal (bonos y pasivo) sí se mueve.
