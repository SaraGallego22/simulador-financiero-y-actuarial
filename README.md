...
**Esta columna se normaliza contra lo que la simulación realmente logra, no contra la fórmula nominal de arriba.** `RISK_ADJUSTED_YIELD_MIN`/`MAX` (`src/domain/finance/alm.ts`) son el rendimiento ajustado por riesgo **realizado** — no el de esta tabla — de un portafolio 100% en el peor y el mejor instrumento del menú (LIQ y TES UVR, respectivamente), corridos completos a través de `almSim()`/`scoreFinanciero()`. Antes de corregir un desfase de un mes en cómo se acumula el rendimiento mes a mes (ver más abajo), lo *realizado* quedaba muy por debajo de lo nominal para casi cualquier portafolio — con esa corrección, ambos números ya casi coinciden (diferencia de una centésima o menos).
