const SECTIONS = [
  ...
  },
  {
    ...
    body: "El capital de solvencia requerido (RK) combina riesgo de suscripción (primas + reservas), riesgo financiero y riesgo operacional (sobre primas), agregados con una matriz de correlación (similar en espíritu a Solvencia II). El riesgo financiero ya no es un porcentaje plano sobre las inversiones: se escala por la volatilidad realmente realizada del portafolio del equipo, relativa al promedio del menú de instrumentos — un equipo que concentró su portafolio en acciones (el instrumento más volátil) paga un capital requerido mayor que uno con el mismo monto invertido en instrumentos más estables, sin importar el rendimiento nominal obtenido. Los fondos propios (patrimonio) también bajan directamente si el equipo tuvo que comprometer Capital Social en el ALM del Día 1/Día 2 para cubrir una brecha de caja — dos vías independientes por las que una mala decisión de portafolio le cuesta en el margen de solvencia. El margen de solvencia es fondos propios sobre RK; el dividendo sugerido es el excedente de fondos propios sobre el RK objetivo. Adicionalmente, cada equipo nombra hasta 3 sectores — cada uno un cruce de dos variables de riesgo, no un nivel aislado — a priorizar para crecer y hasta 3 para disminuir, en orden de prioridad; se califica contra el ranking real de sectores de todo el mercado, no contra la cartera propia de cada equipo, así que la falta de información sobre el resto del mercado es parte del reto.",
  },
  {
