const SECTIONS = [
  ...
  },
  {
    ...
    body: "El menú de instrumentos tiene distintos plazos y rendimientos. El equipo no hace una asignación estática: construye un árbol de decisiones — reparte 100 entre los instrumentos y, para cada tramo, decide qué pasa al vencer (mantener en caja, repetir indefinidamente, o reasignar entre nuevos instrumentos, cada uno con su propia decisión). LIQ y acciones no tienen plazo contractual propio, así que el equipo les fija un vencimiento personalizado. La simulación mensual (60 meses) lleva un estado de caja de 6 columnas contra una Caja Mínima obligatoria cada mes; solo «mantener en caja» hace que el dinero de una inversión vuelva a esa fila — «repetir»/«reasignar» nunca ayudan a la caja mientras sigan en ese ciclo, aunque sigan rindiendo. LIQ es la excepción: siempre se puede retirar para cubrir una brecha, sin importar su propio vencimiento; acciones, en cambio, quedan genuinamente ilíquidas hasta el suyo — la primera vez que una posición en acciones puede volver a ser caja. La nota de calce evalúa el match en todo el horizonte, no solo el peor mes: penaliza tanto la brecha máxima (riesgo de cola) como la brecha promedio acumulada (descalce crónico). Buen calce + buen rendimiento + buena liquidez de corto plazo = nota alta; encadenar todo en reinversiones muy largas deja sin caja para pagar siniestros en curso, toda en caja calza perfecto pero rinde poco.",
  },
];
