...
export default async function AdminGuiaPasantePage({ params }: { params: Promise<{ n: string }> }) {
  ...
  return (
    <main className="print-light mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 bg-[var(--color-background)] p-6 print:bg-white print:p-0 sm:p-8">
      <div className="flex items-center justify-between print:hidden">
        <Link href={`/admin/day/${day}`} className="text-sm text-[var(--color-brand-blue-accent)] underline">
          ← Volver al Día {day}
        </Link>
        <PrintButton />
      </div>
