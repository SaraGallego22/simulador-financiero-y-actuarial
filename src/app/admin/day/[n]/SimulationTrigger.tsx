...
export function SimulationTrigger({ day, defaultCuotaPercent }: { day: number; defaultCuotaPercent: number }) {
  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--color-brand-gray-light)] border-t-4 border-t-[var(--color-brand-cyan)] bg-[var(--color-brand-surface)] p-5">
      ...
      <button
        onClick={run}
        disabled={loading}
        className="rounded-full bg-[var(--color-brand-blue)] shadow-[var(--shadow-sm)] transition-all hover:-translate-y-0.5 active:translate-y-0 hover:shadow-[var(--shadow-md)] px-4 py-2 text-sm font-medium text-white hover:brightness-110 disabled:opacity-50"
      >
