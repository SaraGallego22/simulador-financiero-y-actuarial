...
describe("maxPremiumForCapital", () => {
  ...
  it("solves the margin exactly to CAPACITY_TARGET_MARGIN at the computed premium", () => {
    ...
    const FZ_opPct = 0.03;
    const FZ_diasRotacionCxp = 30;
    const FZ_gastosTotalPct = 0.04 + 0.15 + 0.06;
    const FZ_cajaPct = 0.15;
    const FZ_diasRotacionCxc = 30;
    ...
    const cxp = (FZ_diasRotacionCxp / 365) * FZ_gastosTotalPct * pStar;
    const inversiones = reservas + cxp + availableCapital - FZ_cajaPct * pStar - (FZ_diasRotacionCxc * pStar) / 365;
