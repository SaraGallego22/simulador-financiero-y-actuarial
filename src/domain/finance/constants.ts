...
export const FZ = {
  ...
  rpndPct: 0.2,
  /** Día 3's "Ajuste de siniestralidad A1" P&G line (concepts.ts, p2_ajusteSiniestralidad): a one-time release of Año 1's OWN remaining share of the reserve as of Año 2's close (development.osY1endY2 — not Año 1's full 2027 closing reserve, which is typically much larger by then and could drive the reserve negative), narrated in the Guía del Pasante as an actuarial-team review finding that 2027's remaining unpaid severity was overestimated by this fraction. A REAL event in finBench.ts, not just a reported line — see PnL.ajusteSiniestralidad's doc comment. Independent of what a team itself submitted for Costo de Siniestros A1 on Día 2 — not a correction of a team's own guess. */
  sevRevisionA1Pct: 0.1,
  /** Día 4 EVA (Valor Económico Agregado) deliverable: EVA = Utilidad Neta − Ke (this constant) × capital objetivo, with capital objetivo = `targetMargin` (below) × `solRk` — the same 1.5× RK threshold `div` already uses — rather than the team's own actual `solFp`. Using `solFp` directly let a negative patrimonio (a team that compromised Capital Social) flip the capital charge into a credit, since a negative capital times a positive rate is negative; `solRk` (a sum of non-negative risk charges) never can (see finBench.test.ts's negative-equity EVA case). Ke = 10% for now, matching a plausible cost of equity for an insurer in an emerging market — see the Guía del Pasante's Día 4 for real-world estimation methodologies (CAPM, build-up, dividend growth); revisit if a real rubric calibration says otherwise. */
  costoCapital: 0.1,
};
